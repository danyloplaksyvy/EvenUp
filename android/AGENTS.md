...
Guest passcodes are generated by Android before saving a finalized expense.
Keep passcode generation in domain/sharing logic, pass the generated value through the save request, and show it on the saved/share screen with the share link.
AI readiness, clarification order, participant matching, and total-only allocation belong in domain logic. Persist AI session/default state through repository APIs. Android `SpeechRecognizer` may transcribe locally, but audio must never be uploaded or retained; only editable text reaches the Worker.
...
Saved/share UI should keep the link and four-letter passcode visually separate, but the Android share-sheet message should include both values.
